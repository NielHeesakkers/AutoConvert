  ...
  return new Promise((resolve, reject) => {
    ...
    const doGet = (reqUrl) => {
      const req = https.get(reqUrl, {
        headers: {
          'Api-Key': apiKey,
          'User-Agent': 'AutoConvert v2.0',
          Accept: 'application/json',
        },
        timeout: 10000,
      }, res => {
        if (res.statusCode === 301 || res.statusCode === 302) {
          return doGet(res.headers.location).then(resolve).catch(reject);
        }
        let data = '';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      if (res.statusCode === 301 || res.statusCode === 302) {
        ...
        https.get(res.headers.location, { headers: { Accept: 'application/json' }, timeout: 10000 }, res2 => {
          let data = '';
