...
function hasFfprobe() {
  return new Promise(resolve => {
    execFile('which', ['ffprobe'], { timeout: 3000 }, (err) => resolve(!err));
  });
}

function ffprobeFile(filePath) {
  return new Promise(resolve => {
    execFile('ffprobe', ['-v', 'quiet', '-print_format', 'json', '-show_streams', '-show_format', filePath],
...
function walkDir(dirPath, files = []) {
  try {
    ...
    if (err.code !== 'ENOENT') console.warn(`[library] Cannot read directory ${dirPath}: ${err.message}`);
  }
  return files;
}
...
async function scan() {
  ...
  const useFfprobe = await hasFfprobe();
  ...
  for (const dir of mediaDirs) {
    ...
    for (const f of dirFiles) {
      ...
      } catch (e) { console.warn(`[library] Cannot stat ${f}: ${e.message}`); }
    }
  ...
  for (let i = 0; i < allFiles.length; i++) {
    ...
    } catch (e) { console.warn(`[library] Cannot stat ${filePath}: ${e.message}`); }

    const ext = path.extname(filePath).toLowerCase().replace('.', '');
    stats.byFormat[ext] = (stats.byFormat[ext] || 0) + 1;

    if (useFfprobe) {
      const probe = await ffprobeFile(filePath);
      if (probe) {
...
function loadConversionHistory() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      } catch (e) { console.warn(`[library] Failed to parse report ${file}: ${e.message}`); }
    }

    for (const [date, counts] of dayMap) {
      history.push({ date, ...counts });
    }
  } catch (e) { console.warn(`[library] Failed to load conversion history: ${e.message}`); }
...
function clearCache() {
  try {
    ...
    console.warn(`[library] Failed to clear cache: ${err.message}`);
  }
  return false;
}

module.exports = { scan, getCachedStats, getScanStatus, cancelScan, clearCache };
