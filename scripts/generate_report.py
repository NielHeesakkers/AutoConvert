...
def generate_html(report_dir):
    ...
    html.append(f"Subject: Nieuwe Content voor {date_short}")
...
<h1 style="color: #1a73e8; margin: 0 0 5px 0; font-size: 24px;">Nieuwe Content voor {date_short}</h1>
...
<tr>
  <td style="padding: 15px 20px; text-align: center; border-right: 1px solid #e9ecef;">
    ...
    <div style="font-size: 12px; color: #666; text-transform: uppercase;">Geconverteerd</div>
  </td>
  <td style="padding: 15px 20px; text-align: center; border-right: 1px solid #e9ecef;">
    <div style="font-size: 28px; font-weight: bold; color: {'#c62828' if failed_items else '#2e7d32'};">{len(failed_items)}</div>
    <div style="font-size: 12px; color: #666; text-transform: uppercase;">Mislukt</div>
  </td>
  <td style="padding: 15px 20px; text-align: center; border-right: 1px solid #e9ecef;">
    <div style="font-size: 28px; font-weight: bold; color: #555;">{len(dupes)}</div>
    <div style="font-size: 12px; color: #666; text-transform: uppercase;">Duplicaten</div>
  </td>
  <td style="padding: 15px 20px; text-align: center;">
    <div style="font-size: 28px; font-weight: bold; color: #999;">{skipped}</div>
    <div style="font-size: 12px; color: #666; text-transform: uppercase;">Overgeslagen</div>
  </td>
</tr>
</table>""")

    if converted_items:
        html.append('<h2 style="color: #2e7d32; font-size: 18px; margin: 25px 0 15px 0;">Geconverteerd</h2>')
...
<div style="display: flex; align-items: flex-start; padding: 12px 0; border-bottom: 1px solid #f0f0f0;">
  ...
  <div style="flex-grow: 1; min-width: 0;">
    ...
    if failed_items:
        html.append('<h2 style="color: #c62828; font-size: 18px; margin: 25px 0 15px 0;">Mislukt</h2>')
...
<div style="display: flex; align-items: center; padding: 10px; margin-bottom: 8px; background: #fef2f2; border-radius: 8px; border-left: 3px solid #c62828;">
  ...
  <div>
    ...
    if dupes:
        html.append('<h2 style="font-size: 18px; margin: 25px 0 15px 0; color: #555;">Verwijderde duplicaten</h2>')
...
<div style="text-align: center; padding: 40px 0;">
  ...
  <div style="font-size: 16px; color: #666;">Geen MKV bestanden gevonden. Alles is up-to-date!</div>
</div>""")

    html.append(f"""
<hr style="border: none; border-top: 1px solid #eee; margin: 25px 0 15px 0;">
<p style="color: #aaa; font-size: 11px; line-height: 1.6;">
  Automatisch gegenereerd door AutoConvert<br>
